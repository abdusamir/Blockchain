...
class Blockchain:
    ...
    def proof_of_work(self, previous_proof,block):
        new_proof = 1
        check_proof = False
        ...
        while check_proof is False:
            ...
            if hash_operation[:5] == '00000':
                check_proof = True
                print(hash_operation,block['index'])
            else:
                new_proof += 1
...
def mine_block():
    if len(blockchain.chain)==0:
        ...
        proof = blockchain.proof_of_work(0,block)
    else :
        previous_block = blockchain.print_previous_block()
        previous_proof = previous_block['proof']
        previous_hash = blockchain.hash(previous_block)
        block = blockchain.create_block(15, previous_hash)
        proof = blockchain.proof_of_work(previous_proof,block)
